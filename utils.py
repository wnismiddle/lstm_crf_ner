...
def test_error_analysis(path, logger):

    predict_file = os.path.join(path, "ner_predict.utf8")
    error_dict = {}
    with open(predict_file, "r", encoding='utf8') as f:
        ...
        for block in f.readlines():
            ...
            word.append(block_split[0])
            y.append(block_split[1])
            y_predict.append(block_split[2])

            if y_predict[i] != y[i] and str(y[i]).startswith('B'):
                wrong_start_index = i
                flag = True
            elif flag == True and str(y[i]).startswith('O'):
                wrong_end_index = i
                wrong_type = str(y[wrong_start_index]).split('-')[-1]
                if wrong_type in error_dict:
                    error_dict[wrong_type] += 1
                else:
                    error_dict[wrong_type] = 1
                logger.debug(''.join(word[wrong_start_index: wrong_end_index]))
                logger.debug(''.join(y[wrong_start_index: wrong_end_index]))
                logger.debug(''.join(y_predict[wrong_start_index: wrong_end_index]))
                flag = False
...
def save_model(sess, model, path, logger, current_epoch, loss, remark = ''):
    checkpoint_path = os.path.join(path, "ner.epoch")
    model.saver.save(sess, checkpoint_path + str(current_epoch) + "_loss_" + str(loss) + remark)
...
def result_to_json_simple(string, tags):
    item = {"string": string, "entities": []}
    entity_name = ""
    entity_start = 0
    idx = 0
    for char, tag in zip(string, tags):
        if tag[0] == "B":
            ...
            if entity_name != '':
                item["entities"].append({"word": entity_name, "start": entity_start, "end": idx + 1, "type": type})
            entity_name = ''
            entity_start = -1
        idx += 1
    return item
